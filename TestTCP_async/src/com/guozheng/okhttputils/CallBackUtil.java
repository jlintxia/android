...
public abstract class CallBackUtil<T> {
	...
	public void onError(final byte cmd, final Call call, final Exception e) {
		mMainHandler.post(new Runnable() {
			@Override
			public void run() {
				onFailure(cmd, call, e);
			}
		});
	};

	public void onSeccess(final byte cmd, Call call, Response response) {
	 ...
	 * 访问网络失败后被调用，执行在UI线程
	 */
	public abstract void onFailure(byte cmd, Call call, Exception e);

	/**
	 *
	 * 访问网络成功后被调用，执行在UI线程
	 */
	public abstract void onResponse(byte cmd, T response);
